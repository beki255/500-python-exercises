...
class ActivityStack:
    ...
    def add_activity(self, activity: Activity):
        if len(self.stack) >= self.max_size:
            self.stack.pop(0)  # Remove oldest activity
        self.stack.append(activity)  # Add new activity to top

    def get_all(self) -> List[Activity]:
        return self.stack[::-1]  # Latest on top
...
class ApplicationQueue:
    ...
    def add_application(self, app: Application):
        ...
        heapq.heappush(self.heap, (-app.priority, app.timestamp, app))  # FIFO tiebreaker

    def get_applications_for_job(self, job_id: int) -> List[Application]:
        # MODIFIED: Explicitly return only applications for the specified job_id
...
class SmartJobPortal(tk.Tk):
    ...
    def view_job_applicants(self, job_id: int):
        try:
            ...
            # MODIFIED: Explicitly fetch only applications for job_id
    ...
    def send_invite_to_applicant(self, job_id: int, user_id: str):
        try:
            ...
            if not job:
                messagebox.showerror("Error", "Job not found")
                return
            # MODIFIED: Explicitly verify applicant has applied to this job
            app = next(
                (
                    a
            ...
            if not app:
                messagebox.showerror("Error", "Applicant has not applied to this job")
                return
    ...
    def send_interview_invites(self):
        try:
            if not self.selected_job_id:
                ...
                return
            job_id = self.selected_job_id
            # MODIFIED: Fetch only applications for the selected job
            apps = self.app_queue.get_applications_for_job(job_id)
            if not apps:
                messagebox.showinfo("Info", "No applications found for this job")
                return
            ...
            def refresh_applicants():
                try:
                    ...
                    # MODIFIED: Only show uninvited applicants for this job
                    uninvited_apps = [
                        app
                        for app in apps
